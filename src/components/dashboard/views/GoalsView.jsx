                    ...
                    return (
                        <div key={g.id} style={{
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 14 }}>
                                <span style={{ color: "var(--color-muted)" }}>{t("goals.collected")}: <span style={{ color: "#fff", fontWeight: 600 }}>{fmtRp(g.current)}</span></span>
                                {!done && <span style={{ color: g.color, fontWeight: 600 }}>{t("goals.remaining")}: {fmtRp(g.target - g.current)}</span>}
                            </div>
                            {/* Actions */}
                            <div style={{ display: "flex", gap: 8 }}>
                                <button onClick={() => openEdit(g)} style={{ flex: 1, padding: "7px 0", borderRadius: 8, border: "1px solid rgba(96,252,198,.2)", background: "rgba(96,252,198,.08)", color: "var(--color-primary)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>✏️ Edit</button>
