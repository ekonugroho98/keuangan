...
const COLOR_OPTIONS = ["#ff716c","#f97316","#f59e0b","#60fcc6","#19ce9b","#4FC3F7","#60fcc6","#ec4899","#14b8a6","#76747e","#a855f7","#22c55e"];
        ...
        <div style={{ animation: "fadeIn .4s" }}>
            ...
            {debts.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
                    <div style={{ background: "rgba(255,113,108,.08)", border: "1px solid rgba(255,113,108,.2)", borderRadius: 12, padding: "14px 18px" }}>
                        <div style={{ fontSize: 11, color: "#acaab4", marginBottom: 4 }}>{t("debt.totalSisa")}</div>
                    ...
                    <div style={{ background: "rgba(245,158,11,.08)", border: "1px solid rgba(245,158,11,.2)", borderRadius: 12, padding: "14px 18px" }}>
                        <div style={{ fontSize: 11, color: "#acaab4", marginBottom: 4 }}>{t("debt.totalCicilan")}</div>
            ...
            {debts.length === 0 && (
                <div style={{ background: "rgba(25,25,33,.6)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, padding: "48px 24px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 600, color: "#acaab4", marginBottom: 6 }}>{t("debt.noData")}</div>
                    <div style={{ fontSize: 13, color: "#48474f", marginBottom: 20 }}>{t("debt.noDataSub")}</div>
                    <button onClick={openAdd} style={{ padding: "10px 24px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#ff716c,#e04f4f)", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                        {t("debt.addFirst")}
                    </button>
                </div>
            )}
                ...
                {debts.map(d => {
                    ...
                    return (
                        <div key={d.id} style={{
                            ...
                            {lunas && (
                                <div style={{ position: "absolute", top: 12, right: 12, background: "rgba(96,252,198,.15)", border: "1px solid rgba(96,252,198,.3)", borderRadius: 8, padding: "3px 10px", fontSize: 10, fontWeight: 700, color: "#60fcc6" }}>
                                    {t("debt.paid_badge")}
                                </div>
                            )}
                            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                                ...
                                <div>
                                    ...
                                    {d.monthly > 0 && <div style={{ fontSize: 11, color: "#76747e" }}>{t("debt.monthly")}: {fmtRp(d.monthly)}/bln</div>}
                                    {d.due_date ? <div style={{ fontSize: 11, color: "#acaab4" }}>{t("debt.dueDate")}: {d.due_date}</div> : null}
                                </div>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                                <span style={{ fontSize: 12, color: "#acaab4" }}>{t("debt.paid")} {pct}%</span>
                            ...
                            <div style={{ height: 8, borderRadius: 4, background: "rgba(255,255,255,.04)", marginBottom: 12 }}>
                                <div style={{ height: "100%", borderRadius: 4, background: lunas ? "#60fcc6" : d.color, width: `${pct}%`, transition: "width 1s" }} />
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 14 }}>
                                <span style={{ color: "#76747e" }}>{t("debt.total")}: {fmtRp(d.total)}</span>
                                <span style={{ color: lunas ? "#60fcc6" : "#ff716c", fontWeight: 700 }}>{t("debt.remaining")}: {fmtRp(d.remaining)}</span>
                            </div>
                            {/* Actions */}
                            <div style={{ display: "flex", gap: 8 }}>
                                <button onClick={() => openEdit(d)} style={{ flex: 1, padding: "7px 0", borderRadius: 8, border: "1px solid rgba(96,252,198,.2)", background: "rgba(96,252,198,.08)", color: "#60fcc6", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>✏️ {t("common.edit")}</button>
                    ...
                    <div onClick={e => e.stopPropagation()} style={{ background: "#0f0f1a", border: "1px solid rgba(255,255,255,.08)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 440, maxHeight: "90vh", overflowY: "auto" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 12, padding: 14, marginBottom: 20 }}>
                            ...
                            <div>
                                ...
                                <div style={{ fontSize: 11, color: "#76747e" }}>{t("debt.remaining")}: {form.remaining ? fmtRp(parseInt(form.remaining)) : "Rp 0"}</div>
                            </div>
                        </div>

                        <label style={{ fontSize: 11, fontWeight: 600, color: "#acaab4", display: "block", marginBottom: 6 }}>{t("debt.nameLabel")}</label>
                        ...
                        <label style={{ fontSize: 11, fontWeight: 600, color: "#acaab4", display: "block", marginBottom: 6 }}>{t("debt.totalLabel")}</label>
                        <input type="number" value={form.total} onChange={e => setForm(p => ({ ...p, total: e.target.value }))} placeholder="50000000"
                            style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, color: "#fff", fontSize: 13, fontFamily: "inherit", outline: "none", marginBottom: 16, boxSizing: "border-box" }} />

                        <label style={{ fontSize: 11, fontWeight: 600, color: "#acaab4", display: "block", marginBottom: 6 }}>{t("debt.remainingLabel")}</label>
                        <input type="number" value={form.remaining} onChange={e => setForm(p => ({ ...p, remaining: e.target.value }))} placeholder="35000000"
                            style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, color: "#fff", fontSize: 13, fontFamily: "inherit", outline: "none", marginBottom: 16, boxSizing: "border-box" }} />

                        <label style={{ fontSize: 11, fontWeight: 600, color: "#acaab4", display: "block", marginBottom: 6 }}>{t("debt.monthlyLabel")}</label>
                        <input type="number" value={form.monthly} onChange={e => setForm(p => ({ ...p, monthly: e.target.value }))} placeholder="1500000"
                            style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, color: "#fff", fontSize: 13, fontFamily: "inherit", outline: "none", marginBottom: 16, boxSizing: "border-box" }} />

                        <label style={{ fontSize: 11, fontWeight: 600, color: "#acaab4", display: "block", marginBottom: 6 }}>{t("debt.dueDateLabel")}</label>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
                            {EMOJI_OPTIONS.map(e => (
                                <button key={e} onClick={() => setForm(p => ({ ...p, icon: e }))}
                                    style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${form.icon === e ? "#ff716c55" : "rgba(255,255,255,.06)"}`, background: form.icon === e ? "rgba(255,113,108,.15)" : "transparent", fontSize: 18, cursor: "pointer" }}>
                                    {e}
                                </button>
                            ))}
                        </div>
                        ...
                        <button onClick={handleSubmit} disabled={!canSubmit}
                            style={{ width: "100%", padding: 12, borderRadius: 12, border: "none", background: !canSubmit ? "rgba(255,255,255,.05)" : "linear-gradient(135deg,#ff716c,#e04f4f)", color: "#fff", fontWeight: 700, fontSize: 13, cursor: !canSubmit ? "not-allowed" : "pointer", opacity: !canSubmit ? .4 : 1, fontFamily: "inherit" }}>
