        ...
        <div style={{ animation: "fadeIn .4s", display: "flex", flexDirection: "column", gap: 22 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                ...
                <div style={{ background: "rgba(96,252,198,.08)", border: "1px solid rgba(96,252,198,.2)", borderRadius: 12, padding: "8px 18px", textAlign: "right" }}>
                    <div style={{ fontSize: 10, color: "#60fcc6", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 2 }}>
                        {t("dash.totalBalance") || "Total Saldo"}
                    </div>
                    <div style={{ fontSize: 20, fontWeight: 800, color: "#60fcc6" }}>{fmtRp(totalBalance)}</div>
                </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14 }}>
                {/* Total Saldo */}
                <div style={{ background: "#1f1f28", borderRadius: 14, padding: "18px 20px", borderLeft: "4px solid #00C896" }}>
                    <p style={{ fontSize: 10, fontWeight: 700, color: "#acaab4", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>
                        {t("dash.totalBalance") || "Total Saldo"}
                    </p>
                    <h3 style={{ fontSize: 20, fontWeight: 800, color: "#00C896", margin: "0 0 4px" }}>{fmtRp(totalBalance)}</h3>
                    <p style={{ fontSize: 10, color: "#8B8BA8", margin: 0 }}>
                        {accounts.length} {t("dash.accountsCount") || "rekening"}
                    </p>
                </div>
                {/* Jumlah Rekening */}
                <div style={{ background: "#1f1f28", borderRadius: 14, padding: "18px 20px", borderLeft: "4px solid #a78bfa" }}>
                    <p style={{ fontSize: 10, fontWeight: 700, color: "#acaab4", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>
                ...
                {/* Bank */}
                <div style={{ background: "#1f1f28", borderRadius: 14, padding: "18px 20px", borderLeft: "4px solid #00C896" }}>
                    <p style={{ fontSize: 10, fontWeight: 700, color: "#acaab4", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>Bank</p>
                    <h3 style={{ fontSize: 32, fontWeight: 800, color: "#00C896", margin: "0 0 4px", lineHeight: 1 }}>{bankCount}</h3>
                    <p style={{ fontSize: 10, color: "#8B8BA8", margin: 0 }}>
                ...
                {/* E-Wallet */}
                <div style={{ background: "#1f1f28", borderRadius: 14, padding: "18px 20px", borderLeft: "4px solid #a78bfa" }}>
                    <p style={{ fontSize: 10, fontWeight: 700, color: "#acaab4", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>E-Wallet</p>
                ...
                {/* Tunai */}
                <div style={{ background: "#1f1f28", borderRadius: 14, padding: "18px 20px", borderLeft: "4px solid #f59e0b" }}>
                    <p style={{ fontSize: 10, fontWeight: 700, color: "#acaab4", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>{t("acc.cash") || "Tunai"}</p>
            ...
            {/* ── Account Cards Grid ── */}
            <div>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: "#efecf7", margin: "0 0 14px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 16 }}>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div style={{ marginBottom: 14 }}>
                                    <div style={{ fontSize: 16, fontWeight: 700, color: "#efecf7", marginBottom: 2 }}>{a.name}</div>
                    ...
                    >
                        <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(96,252,198,.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, color: "#60fcc6" }}>+</div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#60fcc6" }}>{t("acc.addNew")?.replace("+ ", "") || "Tambah Rekening"}</div>
                        <div style={{ fontSize: 11, color: "#8B8BA8" }}>{t("acc.addNewSub") || "Bank, e-wallet, atau tunai"}</div>
                    </div>
                </div>
            </div>
            ...
            {accounts.length > 0 && (
                <div style={{ background: "#1f1f28", borderRadius: 18, padding: "22px 22px 18px", border: "1px solid rgba(255,255,255,.06)" }}>
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: "#efecf7", margin: "0 0 16px" }}>
                    ...
                    {recentTx.length === 0 ? (
                        <div style={{ textAlign: "center", padding: "32px 0", color: "#475569" }}>
                            ...
                            <p style={{ fontSize: 12, margin: 0, color: "#acaab4" }}>{t("tx.noTxPeriod") || "Belum ada transaksi"}</p>
                        </div>
                    ) : (
                        <>
                            {recentTx.map((tx, idx) => {
                                    ...
                                    <div key={tx.id} style={{
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                                            ...
                                            <div style={{ minWidth: 0 }}>
                                                <div style={{ fontSize: 13, fontWeight: 600, color: "#efecf7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.note}</div>
                                                <div style={{ fontSize: 10, color: "#76747e" }}>{fmtDate(tx.date)} · {tCat(tx.category)}</div>
                                            </div>
                                        </div>
                            ...
                            <button
                                onClick={() => setActiveMenu && setActiveMenu("transaksi")}
                                style={{ width: "100%", marginTop: 12, padding: "9px 0", borderRadius: 10, border: "1px dashed rgba(96,252,198,.2)", background: "transparent", color: "#60fcc6", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
                            >
